<script lang="ts">
	...
	const entries: Entry[] = [
		{
			...
			col: 4,
			answer: 'OGERTA',
			clue: 'Emri i ministres së njohur si “Madame Covid”.'
		},
		{
			id: 'kryemadhi',
			number: 2,
			...
			col: 4,
			answer: 'KRYEMADHI',
			clue: 'E akuzuar për 106 mijë euro në ndërhyrje estetike dhe botoks.'
		},
		{
			id: 'balluku',
			number: 3,
			...
			col: 4,
			answer: 'BALLUKU',
			clue: '“Vejusha e zezë e Rilindjes”.'
		},
		{
			id: 'klotilda',
			number: 4,
			...
			col: 2,
			answer: 'KLOTILDA',
			clue: 'Emri i deputetes që kërcënoi dëshmitarët e dosjes Veliaj.'
		},
		{
			id: 'evis',
			number: 5,
			...
			col: 5,
			answer: 'EVIS',
			clue: 'Emri i ministres që bën të pamundurën të mundur, derisa e mundura të bëhet e pamundur.'
		},
		{
			id: 'qato',
			number: 6,
			...
			col: 5,
			answer: 'QATO',
			clue: 'Ish-kreu i ARRSH-së, në gjykim pas akuzave të SPAK-ut.'
		},
		{
			id: 'elisa',
			number: 7,
			...
			col: 5,
			answer: 'ELISA',
			clue: 'Emri i “Mamicës së partisë”.'
		},
		{
			id: 'vokshi',
			number: 8,
			...
			col: 2,
			answer: 'VOKSHI',
			clue: 'Kryetarja legjendare e Lidhjes Demokratike të Gruas prej 2009.'
		},
		{
			id: 'duma',
			number: 9,
			...
			col: 5,
			answer: 'DUMA',
			clue: 'Femra që kur flet, transformon banalitetin në epifani leksiko-semantike dhe rikonceptualizon paradigmën retorike të vetë fjalës “asgjë”.'
		},
		{
			id: 'kociu',
			number: 10,
			...
			col: 1,
			answer: 'KOCIU',
			clue: '“Elbasani krah Amsterdamit dhe Madridit”.'
		},
		{
			id: 'xhacka',
			number: 11,
			...
			col: 3,
			answer: 'XHACKA',
			clue: 'Bashkëshorti i saj, ish-deputet, investitor strategjik në konflikt interesi.'
		},
		{
			id: 'kumbaro',
			number: 12,
			...
			col: 3,
			answer: 'KUMBARO',
			clue: '“Pikë e vogël”, jo aq e vogël.'
		}
	];
	...
	let activeEntryId = $state('ogerta');
	...
	$effect(() => {
		...
		celebrate();
		checked = true;
		status = 'Bravo! Fjalëkryqi u plotësua saktë. Vertikalja është “GRATË QË S’DUAM”.';
	});
	...
	function focusCell(cell: Cell) {
		document.getElementById(`crossword-six-cell-${cell.key}`)?.focus();
	}
	...
	<div class="crossword-game__play-area">
		...
		<div class="crossword-board-shell">
			<div
				...
				role="group"
				aria-label="Fjalëkryqi i Botimit VI, dymbëdhjetë përgjigje horizontale dhe një kolonë vertikale e theksuar"
			>
				{#each board as square (`${square.row}-${square.col}`)}
					{#if square.cell}
						...
						>
							...
							<input
								id={`crossword-six-cell-${cell.key}`}
								type="text"
								inputmode="text"
								maxlength="1"
								autocomplete="off"
		...
		<p class="crossword-game__hint">
			Plotëso përgjigjet horizontalisht. Shkronjat në kolonën e theksuar zbulojnë frazën përfundimtare.
		</p>
	</div>

	<div class="crossword-clues">
		<section aria-labelledby="crossword-six-across-title">
			<h3 id="crossword-six-across-title">Horizontal</h3>
			<ol>
