...
import { isRecord, jsonResponse, parseFingerprint, toPublicIdea, voteDeltas } from '@lib/functions';
...
function resolveFingerprint(request: Request, body: Record<string, unknown> | null) {
	return parseFingerprint(body?.fingerprint) || parseFingerprint(request.headers.get('X-Device-Fingerprint'));
}
...
export const POST = async (context: VoteContext) => {
	...
	const rating = parseVoteValue(body.value) || parseVoteValue(body.rating) || parseVoteValue(body.vote);
